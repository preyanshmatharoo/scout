...
class EC2Facade(AWSBaseFacade):
    ...
    async def get_instance_user_data(self, region: str, instance_id: str):
        ...
        try:
            ...
            print("^^^^^^^^^")
            print(user_data_response['UserData']['Value'])
            print("^^^^^^^^^")
    ...
    async def get_instance_api_termination(self, region: str, instance_id: str):
        ec2_client = AWSFacadeUtils.get_client('ec2', self.session, region)
        try:
            user_data_response = await run_concurrently(
            ...
            print()
            print(user_data_response['DisableApiTermination']['Value'])
            print()
            print(str(user_data_response['DisableApiTermination']['Value']))
            print("**** Attribute End *****")
        except Exception as e:
            print_exception(
                'Failed to describe EC2 instance attributes: {}'.format(e))
            ...
            if 'Value' not in user_data_response['DisableApiTermination'].keys():
                return None
